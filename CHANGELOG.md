...
- **certbot:** new `dns-cloudflare` mode — DNS-01 validation via a Cloudflare
  ...
  to reload the webserver on renewal. Challenge propagation wait is
  `certbot_dns_propagation_seconds` (default 60 — the plugin's 10s default
  intermittently fails multi-domain certificates).
